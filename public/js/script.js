...
function categoryProductsHandler(categoryId, start, loadMore) {
    ...
    fetch("categoryProductsHandler", {
        ...
        .finally(() => {
            ...
            document
                .querySelector(".products")
                .classList.remove("products-blur");
        });
}
// 1 секция
document
...
document
    .querySelector(".cart-products-add-del")
    ?.addEventListener("click", addDelToCart);
...
document
    .querySelector(".cart-table-warp table")
...
function deleteFromFavouritesHandler(elem, body) {
    fetch("deleteToFavourites", {
        ...
        .then((resp) => {
            if (!resp.ok) {
                throw new Error(
                    "Ошибка удаления из избранного. Попробуйте позже"
                );
            }
            return resp.json();
        })
          ...
          `;
                ...
                if (document.getElementById("alert-add-product")) {
                    ...
                    if (elem.matches(".add-card")) {
                        ...
                        elem.closest(".add-card").insertAdjacentHTML(
                            "beforebegin",
                            alert
                        );
                        alertElem =
                            elem.closest(".add-card").previousElementSibling;
                    }
                    deleteAlert(alertElem);
                }
                document.querySelector(".shopping-card span").textContent =
                    Number(
                        document.querySelector(".shopping-card span")
                            .textContent
                    ) + 1;
            }
        })
