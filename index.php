<?php include_once 'handlers/api.php'; ?>
<?php // include_once 'handlers/favourites.php'; 
?>
                ...
                <nav class="navbar navbar-toggleable-md navbar-inverse fixed-top bg-inverse">
                    ...
                    <div class="collapse navbar-collapse" id="navbarCollapse">
                        ...
                        <ul class="navbar-nav mr-2">
                            <li class="nav-item">
                                <a class="nav-link" href="tech-favourites.html"><img style="filter: invert(100%);" src="images/icons8-favorite-folder-50.png" width="30"></a>
                            </li>
                        </ul>
                    </div>
                </nav>
        ...
        <section class="section first-section">
            <div class="container-fluid">
                <div class="masonry-blog clearfix">
                    <!-- релизация 3 блоков fitst second last-->
                    <? $i = 0 ?>
                    <? foreach ($posts as $category_name => $post) : ?>
                        ...
                        <div class="<?= $class ?>-slot">
                            <div class="masonry-box post-media">
                                <!--  @getimagesize - уберёт ошибки критические при необнаружении картинки -->
                                <? $url = @getimagesize($post['urlToImage']) ? $post['urlToImage'] : 'images/no-image.png'; ?>
                                <img src="<?= $url ?>" alt="" class="img-fluid">
                                <div class="shadoweffect">
                                    <div class="shadow-desc">
                                        <div class="blog-meta">
                                            <span class="bg-orange"><?= $category_name ?></span>
                                            <h4><a target="_blank" href="<?= $post['url'] ?>" title=""><?= $post['title'] ?></a></h4>
                                            <small><?= date('d.m.Y', strtotime($post['publishedAt'])) ?></small>
                ...
                <div class="row">
                    <div class="col-lg-9 col-md-12 col-sm-12 col-xs-12">
                        <div class="page-wrapper">
                            ...
                            <!-- ДЕЛЕГИРОВАНИЕ СОБЫТИЙ ВЕШАЕМ ТУТ -->
                            <div class="blog-list clearfix">
                                <?php for ($i = 0; $i < 10; $i++) :
                                    $index_category = rand(0, count($data) - 1);
                                    $post_index = rand(0, count($data[$categories[$index_category]]) - 1);
                                    $categoriy = $categories[$index_category];
                                    $post = $data[$categoriy][$post_index];
                                ?>
                                    <!-- новые рандомные посты -->
                                    <? $url = @getimagesize($post['urlToImage']) ? $post['urlToImage'] : 'images/no-image.png'; ?>

                                    <div class="blog-box row">
                                        <div class="col-md-4">
                                            <div class="post-media">
                                                <a target="_blank" href="<?= $post['url'] ?>" title="">
                                                    <img src="<?= $url; ?>" alt="" class="img-fluid">
                                                    <div class="hovereffect"></div>
                                                </a>
                                            </div><!-- end media -->
                                            <!-- ?url= позволяет поймать имя url через GET / POST -->
                                            <!-- PHP -->
                                            <!-- <a href="?category=<?= $categoriy ?>&url=<?= $post['url'] ?>" style="cursor: pointer;" class="btn btn-primary">Add to favourites</a> -->
                                            <!-- JS -->
                                            <a href="" style="cursor: pointer;" class="btn btn-primary add-favourite" data-category="<?= $cat ?>" data-url="<?= $post['url'] ?>">Add to favourites</a>
                                        ...
                                        <div class="blog-meta big-meta col-md-8">
                                            ...
                                            <small class="firstsmall"><?= $category_name ?></small>
                                            <small><?= date('d.m.Y', strtotime($post['publishedAt'])) ?></small>
